...
public class MainController {
    ...
    @FXML public Button startBotNow;
    ...
    public void handlePublicDiscord() {
        if (Desktop.isDesktopSupported()) {
            try {
                URI discord = new URI("https://discord.gg/FyuGTd3");
    ...
    public void handleGithubPage() {
        if (Desktop.isDesktopSupported()) {
            try {
                URI discord = new URI("https://github.com/NaCorliss/Promethium-Engine");
                Desktop.getDesktop().browse(discord);
            } catch (URISyntaxException | IOException e) {
                System.out.println("Error: Could Not Open URL In Browser");
            }
    ...
    public void handleStartBotNow() {
        if (Desktop.isDesktopSupported()) {
            try {
                URI discord = new URI("http://hentaihaven.org/");
                Desktop.getDesktop().browse(discord);
            } catch (URISyntaxException | IOException e) {
                System.out.println("Error: Could Not Open URL In Browser");
            }
